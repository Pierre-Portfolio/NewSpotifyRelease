...
// dalle ordinaire ; gavée (3) elle vire au rouge, TIRE sur le doodler, RAMPE vers lui comme une
// 👀 Vivante et le TUE au contact (13.2.9, voir `doodleTamaMad`).
...
// (D_ALIVE_V, D_ALIVE_SPAN) plutôt que d'en refaire une à côté — et il TUE AU CONTACT.
// ⚠ La colère est le SEUL état létal : affamé, repu et rassasié restent ce qu'ils étaient.
// ⚠ `doodleTamaMad` se lit sur le type EFFECTIF : un 🃏 Casino qui a tiré le tamagotchi rampe
// et tue comme lui, sinon la copie aurait été une pâle imitation.
...
// ⚠ 12.8.2 — LA CABINE NE RENONCE PLUS (demande utilisateur : « elle doit te déposer sur la
// PROCHAINE case TARDIS »). Le plafond valait 5 s : la tuile TARDIS étant rare, la cabine
// abandonnait presque toujours avant d'en croiser une et reposait le doodler sur la première
// dalle venue — le voyage n'allait jamais nulle part. Tant qu'on est dans le biome, elle monte
// AUSSI LONGTEMPS QU'IL LE FAUT ; c'est le changement de BIOME qui borne le voyage, et lui seul.
// ⚠ D_TARDIS_MAX subsiste en pur garde-fou anti-blocage (60 s), pour la partie où le palier ne
// changerait jamais. Il déclenche exactement la même sortie qu'un changement de biome.
// ⚠ Sorti du biome, la destination n'est PAS « la première dalle venue » mais la première
// VERTE OU BLEUE au-dessus : on sort d'un sauvetage, on ne doit pas être reposé dans un piège.
...
// biome. Une fois sorti — ou au garde-fou — seules les dalles VERTES ou BLEUES sont acceptées :
// une dalle « quelconque » pouvait être une pique ou une cassante, et le sauvetage se terminait
// alors par une mort. ⚠ Type EFFECTIF : un 🃏 Casino déguisé en verte n'en est pas une.
function doodleTardisTarget(s, onlyTardis, from) {
  ...
  for (const q of s.platforms) {
    ...
    if (onlyTardis ? et !== 'tardis' : (q.type !== et || (et !== 'green' && et !== 'blue'))) continue;
...
//   • Il apparaît dès D_PRISM_FROM, soit après CINQ biomes différents (demande utilisateur,
//     12.9.9). Les 1000 premiers points ne comptent pas : ils se passent au 📄 Départ, qui n'est
//     pas un biome. Le 1er biome couvre 1000→2000, le 5e 5000→6000 : c'est donc à 6000 points,
//     et pas avant, qu'on a « fait 5 biomes différents ».
...
// ⚠ 12.9.2 — JAMAIS UNE TUILE DE BIOME (demande utilisateur), alors qu'elle savait en copier
// depuis 12.5.4. Une tuile de biome emportée hors de son biome se retrouvait partout ensuite,
// et le 🃏 Casino la ressortait à son tour ; le vivier est donc le seul catalogue débloquable,
// exactement celui des paliers. C'est aussi ce qui rend le tirage UNIFORME : les deux poids de
// copie (D_COPY_W_BIOME / D_COPY_W_RARE) ne pondéraient que ce qui vient d'en partir.
...
// ⚠ LE VOYAGE A ÉTÉ RALENTI DEUX FOIS (demandes utilisateur) : ÷4 en 13.2.4, puis encore
// −50 % en 13.3.9 — 2 → 16 frames par dalle. Écrit `8 * 2` et non `16` : c'est le facteur
// demandé qui doit rester lisible, pas le résultat.
// ⚠ Les traînées et les doodlers fantômes DÉRIVENT de ce pas (`D_CREA_TP_STEP * 5`, `* 4`) :
// ils s'allongent d'eux-mêmes, il n'y a jamais rien à réajuster à côté.
// ⚠ Le garde-fou D_CREA_TP_LIFE (600) couvre toujours largement le voyage : 10 dalles × 16 =
// 160 frames. Il faudrait un pas au-delà de 60 pour qu'il coupe un voyage normal.
const D_CREA_TP_STEP  = 8 * 2;  // une dalle toutes les 16 frames (3,75/s) — encore ÷2 (13.3.9)
