...
| `StatsPanel` | **Section Stats** (heading `STATS`, onglet desktop `stats` + onglet mobile `statspanel`, **juste au-dessus d'À propos**) — **sections repliables `StatsCollapse`** : **🎵 MUSIQUES** (fermée par défaut : restantes, **temps d'écoute** (restant à écouter) et **temps total écouté** — tous deux au format **« X Mois X Jours X Heure »** (helper local `fmtMonthsDaysHours`), mois, année, all-time, **% titres likés** via l'app), **✅ TO DO** (fermée par défaut : tâches terminées de la journée / du mois / de l'année via `todoDoneStats()`, + **★ Tâches compliquées** = total des tâches favorites effectuées), **🎬 STATS FILM** (fermée par défaut, couleur `TVTIME_COLOR` : **films vus**, **épisodes vus** (+ nb de séries entamées), **temps films**, **temps séries**, **temps restant films** et **temps restant séries** — via `tvTimeStats()`, 100% depuis le cache localStorage TV Time, **0 requête API** ; temps séries = `watchedEps × epRuntime` par série ; **temps restant** = films non vus (`movieRemainMin`) + épisodes non vus `(totalEps − watchedEps) × epRuntime` (`epRemainMin`). Les 4 temps sont formatés en **« X Mois X Jours X Heure »** via le helper local `fmtMonthsDaysHours(ms)` de `StatsPanel` (mois = 30 j)), **🗺️ STATS MAPS** (fermée par défaut, couleur `MAPS_COLOR` : **distance parcourue** aujourd'hui / ce mois-ci / cette année / depuis toujours + nb trajets/jours suivis + **🥇 mode préféré** — via `mapsStats()`, 100% localStorage `spotifyplus_maps` (cache mémoire `_mapsCache`, 0 requête)), **📈 GRAPHIQUE** (`ListeningCharts`) et **📊 STATS AVANCÉES** (`AdvancedStats`). Lit `listenStats` + `now` du store. `StatsCollapse({ title, color, defaultOpen, children })` = header cliquable (chevron ▶ qui pivote) + contenu conditionnel |
