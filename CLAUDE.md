...
| **Jeux** | `JeuxPanel`, `#39b2d1` | `spotifyplus_game_hi_*`, `tierlist-v1`, `_doodle_hi_reset`, `_hi_purged_v4` | 27 mini-jeux, **tous 100 % locaux et hors ligne**. `useImmersiveLock(true)` ; `useHidePlayerBar(true)` masque le player (affichage seul) ; `useNoPinchZoom(active)` neutralise le pincement (Osu, pendant une manche) — ⚠ écouteurs **non passifs** obligatoires. ⚠ `_doodle_hi_reset` = migration jouée **une seule fois**, l'effacer remet le record à 0. ⚠ Retirer un jeu ⇒ ajouter son id à `REMOVED_GAMES` (cumulatif) et **renommer** `HI_PURGE_LS` (`_v5`…), sinon la purge ne repart pas là où l'ancienne a déjà tourné |
