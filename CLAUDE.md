...
- **⚠ TROIS contextes, pas un** : `useStore()` (données stables) · `useNow()` (titre en cours, change toutes les 5 s) · `useSyncInfo()` (`nextCall`/`logs`/`statusMsg`/`currentArtist`/`eta`, lus par les 3 panneaux du collapse ⚙ Scrapping). **Ne JAMAIS remettre dans `api` une valeur qui bouge à intervalle régulier** : elle recrée l'objet du store et re-rend tous ses abonnés (canvas de jeu compris).
