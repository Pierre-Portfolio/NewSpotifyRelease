...
| `service-worker.js` | **DEUX caches** : `spotifyplus-v<N>` (app shell, bumpé à chaque déploiement) et `spotifyplus-vendor-v1` (fichiers de `vendor/` **+ `manifest.json` et les 2 icônes**, **bumpé seulement quand l'un d'eux change** — `primeVendor()` étant incrémental, AJOUTER une entrée ne demande aucun bump — sinon 1,9 Mo immuables repartaient sur le réseau à chaque déploiement ; `primeVendor()` ne (re)télécharge que ce qui manque ; `doodle.js` et `sport-data.js` sont dans le précache de l'**app shell**, ils changent avec elle). `data/*.json` en **stale-while-revalidate** (cache servi, réseau en fond). **Network-first avec plafond `SHELL_TIMEOUT_MS` (1,5 s)** pour l'app shell (`fetch {cache:'no-cache'}` : revalidation CONDITIONNELLE — bypasse le `max-age=600` de GitHub Pages tout en laissant un 304 éviter les 3,1 Mo ; **jamais `no-store`**, qui refait le téléchargement entier à chaque lancement) : au-delà du délai on sert le cache et le téléchargement continue en fond ; s'il rapporte un ETag différent de la copie servie, `postMessage('shell-updated')` **au seul client servi** (`resultingClientId`) → bandeau `#newver`. Ne cache que `res.ok`, clé normalisée `./index.html` ; **l'app shell = le chemin EXACT de `self.registration.scope` (ou + `index.html`)**, jamais « tout ce qui finit par `/` ». Une ressource locale absente du précache est **mise en cache au premier accès** (dans l'app shell, donc purgée au déploiement suivant) — sauf le worker lui-même. ⚠ Sans cache, on attend le réseau sans plafond. ⚠ Son `activate` supprime tout cache dont le nom ≠ le sien — **sauf `spotifyplus-compiled`** (code compilé, se purge lui-même) **et `VENDOR`**. ⚠ Le cross-origin ne passe plus par le worker du tout. **Bumper la version du cache après changement.** |
| `vendor/react*.production.min.js` | React 18.3.1 auto-hébergé (fichiers du paquet npm, empreintes identiques à unpkg) |
...
| `data/indices.json` | Indices boursiers via `scripts/fetch_indices.py` (Stooq→Yahoo), **même workflow**, séance seulement (06–22 UTC, lun–ven). Comme l'actu, la valeur vient d'un **relais public** : `check_quote` refuse un non-nombre ou un écart > 25 % avec le relevé précédent, et la clé `via` de chaque cours nomme le relais → bandeau `IndicesViaNotice` |
...
| **Mot de Passe** | `GestionMdpPanel`, ambre | `spotifyplus_mdp_vault` | AES-GCM 256 + PBKDF2 600k. ⚠ La clé dérivée est CACHÉE en mémoire (`_mdpKeyCache`, 4 max, vidé au verrouillage/déconnexion) et `mdpEncrypt` REJOUE le sel connu — sinon l'autosave d'une note cryptée repayait 600k itérations à chaque pause de frappe ; **ce qui doit rester neuf à chaque chiffrement, c'est l'IV, pas le sel**. Le blob chiffré contient `{entries, tags}` → suivi par les sauvegardes. Verrouillage auto 10 min. ⚠ Mot de passe oublié = irrécupérable |
