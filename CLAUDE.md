...
| `service-worker.js` | Cache app shell + vendor pour usage offline (v7 : + Leaflet précaché ; clé de cache normalisée, ne cache que les réponses `res.ok`, fetch app shell en `{ cache: 'no-store' }` pour bypasser le cache HTTP `max-age=600` de GitHub Pages) |
