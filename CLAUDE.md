...
- **Les deux logs de refus quota affichent la date/heure absolue de réouverture** (`new Date(until).toLocaleString('fr-FR', { dateStyle:'long', timeStyle:'short' })`) en plus de la durée relative : refus en tête de `startSync` (« prochaine synchro possible le 19 juin 2026 à 14:30 (dans X h Y min) ») et arrêt en cours de boucle (« Prochaine synchro possible le … »)
