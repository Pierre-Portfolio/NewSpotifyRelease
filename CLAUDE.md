...
| `VosEcoutesPanel` | **Section À propos** (heading `À PROPOS`, onglet desktop `ecoutes` + onglet mobile `stats`) — **actions de données**, dans l'ordre : bouton **↥ Restaurer mes données** (**en premier**, input file JSON → `restoreBackup`, confirm avant application), puis **section repliable « ↧ EXPORTER MES DONNÉES »** (`StatsCollapse`, **fermée par défaut**) avec **10 boutons d'export séparés** définis par le tableau module-level `EXPORT_SECTIONS` = 📦 Tous (export complet) / 🎤 Artistes (`artists`) / 🎬 TV Time (`tvtime`+`tmdb_key`) / ✅ To do (`todos`+`todos_done`) / 🗺️ Maps (`maps`) / 📈 Finance (`finance`, historique local des prix) / 🔑 Mot de passe (`mdp_vault`, blob chiffré) / 🔔 Remember (`rappels`+`remembers`) / 📊 Stats (`stats`) / ⚙️ Paramétrage (`modules`, modules activés/désactivés) — chaque bouton appelle `downloadBackup(sec)` : un export partiel = fichier `spotifyplus-<key>-<date>.json` contenant `app/version/exported_at/scope` + les clés choisies (restaurable par ↥ Restaurer, `applyBackup` ignore les clés absentes) ; **seul l'export complet « Tous » pose `spotifyplus_last_backup`** (proposition hebdo). Puis **section repliable « ⚙️ PARAMÉTRAGE »** (`StatsCollapse`) = un **toggle switch** par module basculable (`TOGGLE_MODULES`) via `setModuleEnabled` — active/désactive l'onglet correspondant (desktop & mobile) en direct, réglage inclus dans l'export/import. Puis `DropboxSync` puis `GoogleDriveSync` (juste dessous), bouton **🔄 Réinitialiser le quota 24h** (affiche `X/100` + `· bloqué` si fenêtre active, `confirm()` puis `resetQuota()`), bouton **🗑 Purger les écoutes**, et **texte gris « Version {APP_VERSION} »** centré sous le bouton Purger. (Les chiffres/graphiques ont été déplacés dans `StatsPanel`.) |
