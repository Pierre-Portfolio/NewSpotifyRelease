...
| `ListeningCharts` / `BarChart` | **Toggle Graphique** (dans Stats, sous STATS FILM, via un `StatsCollapse` « 📈 GRAPHIQUE ») — deux bar charts en divs (aucune lib externe → CSP inchangée) : **écoutes / jour** (14 j, depuis `tracks.listened_at` converti en `localtime`) et **tâches terminées / jour** (14 j, depuis `spotifyplus_todos_done`). Helpers module-level `_lastNDays(n)`, `listensPerDay(n)`, `todosPerDay(n)`. `ListeningCharts` recalcule au montage (donc à l'ouverture du collapse) |
