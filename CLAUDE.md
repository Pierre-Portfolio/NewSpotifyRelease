...
| `MobileApp` | Layout mobile — **barre du haut unique** (un peu plus haute, `padding:'13px 14px'`) : **à gauche le profil** (photo `UserChip` + nom + statut `product`, à la place de l'ancien logo « Spotify+ »), **à droite les onglets**. Onglets : **3 principaux affichés** (Scrapping / En attente / ❤ Likés) + **menu « ⋯ »** (overflow) regroupant **Histo / Artistes / Météo / Finance / To do / Mot de Passe / Remember / Stats** (dans cet ordre, Histo et Artistes en tête) + **un bouton « Déconnecter » en rouge tout en bas** (séparé par un trait, appelle `s.logout`). `overflowTabs` = tableau `[clé, libellé, couleur]` — **chaque onglet a une couleur de l'ARC-EN-CIEL** (3e position) appliquée au libellé du dropdown : Histo `#b06cf5` (violet) → Artistes `#6f7bf5` (indigo) → Météo `#4ea3f5` (bleu) → Finance `#2fc1c1` (cyan) → To Do `#45c552` (vert) → **Maps `#e6d53f` (jaune)** → Mot de Passe `#f5b81f` (ambre) → Remember `#f5813c` (orange) → À propos `#e05050` (rouge, = couleur du bouton Déconnecter). Ordre : **Maps est inséré juste sous To Do**. `menuOpen` (state local) ouvre un dropdown avec overlay de fermeture au clic extérieur ; le bouton ⋯ prend la couleur accent quand l'onglet actif est dans l'overflow. (L'ancienne 2e barre profil + bouton Déconnecter a été supprimée.) |
