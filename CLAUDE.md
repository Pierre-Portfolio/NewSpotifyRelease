...
| `ListeningCharts` / `BarChart` | **⚠ Composant défini mais PLUS MONTÉ** (la section « 📈 GRAPHIQUE » a été retirée de `StatsPanel`) — code conservé mais inutilisé. Anciennement : deux bar charts en divs (aucune lib externe → CSP inchangée) : **écoutes / jour** (14 j, depuis `tracks.listened_at` converti en `localtime`) et **tâches terminées / jour** (14 j, depuis `spotifyplus_todos_done`). Helpers module-level `_lastNDays(n)`, `listensPerDay(n)`, `todosPerDay(n)`. `ListeningCharts` recalcule au montage (donc à l'ouverture du collapse) |
