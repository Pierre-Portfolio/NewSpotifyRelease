...
| `vendor/doodle.js` | **Moteur Doodle Jump** (constantes `D_*`, génération, dessin : ~7 000 lignes SANS JSX), **lazy** (`loadDoodle()`), précaché avec l'app shell. ⚠ Script classique ⇒ globales lexicales : **aucune lecture au niveau module** d'`index.html`, aucun nom redéclaré des deux côtés ; `GameDoodle` est le garde (`window.DOODLE_READY`). ⚠ La **✨ Création** n'est PAS dans `D_TILES` (donc jamais débloquée ni tirée) : elle naît de DEUX dalles qui se touchent (`doodleCreaFuse`, une seule à l'écran), puis pilote le doodler (`doodleCreaStep`, **appelée juste avant le test d'atterrissage** — c'est ce test qui joue l'effet de chaque dalle visitée, aucun effet n'est réécrit) |
...
- ⚠ **Un `localStorage` plein ne lève RIEN** (`lsSet` renvoie `false`, `put` avale le throw) : la restauration s'annoncerait réussie sans rien écrire. `applyBackup` renvoie donc `failed[]` → `restoreFailMsg(res)` dans les 4 UI, et `_tvItemsCache` remis à `null`. TV Time est le plus gros objet du backup : il sature en premier.
...
|---|---|---|---|
| **Musique** | `MusiquePanel`, violet | (base SQLite) | 6 collapses `defaultOpen={false}` : ⚙ Scraping · ⏳ En attente · ❤ Likés · 🕘 Historique · 🎤 Artistes suivis · 🔀 Delta (bouton ▶ Analyser + 2 sous-collapses de RÉSULTAT : Delta Playlist, Delta Artiste — un seul scan pour les deux) |
| **Actu** | `ActuPanel`, bleu-indigo | caches 30 min, `spotifyplus_actu_ia`, `_actu_bug` | Actualité · **Bug & Pannes** (⚠ **Downdetector n'a aucune API publique** et bloque les robots : la pastille d'état vient des pages de statut OFFICIELLES — Statuspage, `/api/v2/status.json`, sans clé — listées dans `API_STATUS_HOSTS`, source unique reprise par `DD_SERVICES`, `API_HOST_ID` et la CSP ; downdetector.fr n'est que le lien sortant + un scraping best-effort de sa page d'accueil, masqué s'il échoue) · Cartographie (`MapFrame`) · Fuites de données · Vidéos YouTube (OAuth séparé, lecture dans le `YtFullscreenPlayer` de TV Time). `data/actu.json` prioritaire, repli proxies CORS. `ActuAiSummary` : résumé de la veille écrit **à partir des titres passés dans le prompt** (jamais de la mémoire du modèle, jamais `opts.tools`) ; état module-level `_actuIa`, 1 génération/jour, cache **hors backup**. ⚠ Vue à `ACTU_SEEN_RATIO`, à la fin ou cochée 👁 → `ytSeenMarkAnywhere` + `ytWlDropSeen` ; vidéo trop récente pour la chaîne suivie ⇒ `ytVideoListWith` l'INSÈRE dans ses `videos` (sinon rien ne s'affiche : la fiche ne rend que `videos`), garde-fou `ytItemAcceptsVideo` — `ytOrphanSeen` ne couvre plus que les vidéos sorties de la liste |
| **Alertes** | `RappelPanel`, `#6f7bf5` | `spotifyplus_rappels`, `_rappels_time`, `_rappels_notes`, `_remember_custom`, `_speedrun_todo_added` | Rubriques fixes + rappels persos + sections auto (Frigo, Steam) + calendrier 🗓. `RAPPEL_NOTES` = champs de rubrique en **texte libre**, ⚠ sans date donc **hors du circuit d'alerte** (badge, calendrier, notification et To do dérivent de `RAPPEL_SECTIONS` seul). `RAPPEL_RECUR` / `rappelNextDate` (la date stockée reste celle d'origine). File speedrun non vide ⇒ tâche « Aujourd'hui » du To do (`syncSpeedrunToTodo` : marqueur `srKey`, 1 création/jour, file vidée ⇒ tâche et marqueur retirés) |
...
| **Collection** | `CollectionPanel`, `#5a95e6` | `spotifyplus_collection` | 7 catégories (plus de fourre-tout « Autre » : 💎/⌚/🍷 sont des collapses directs — une catégorie dont l'unique sous-collection porte son id saute le 2e collapse), moteur commun `CollectionSub`. `collAdd` incrémente `qty` sur id existant, `collEdit` corrige une fiche **sans jamais toucher son `id`** (clé de dédoublonnage ET nom du fichier Drive) ; `img` = **URL distante, jamais une image encodée**. ⚠ Les photos d'analyse IA vont dans **Drive `HUB_Pierre/Collection`** (`collPhotoUpload`), nommées avec l'id ASSAINI de la fiche (`collPhotoKey` — les ids portent `:` et des accents) ; index module-level `_collPhotos` listé 1×/session, Drive absent ⇒ photo non conservée, rien d'autre ne change. **Displate / Pop / Montre s'importent depuis les onglets du classeur Finance** (`CollFinanceImport`, `actifsSheetRead` partagé avec le portefeuille crypto) : une colonne = un objet, ⚠ dont la cellule N'EST PAS toujours des euros — le facteur vient du `Total` de la ligne |
...
| **Jeux** | `JeuxPanel`, `#39b2d1` | `spotifyplus_game_hi_*`, `tierlist-v1`, `_flip7`, `_doodle_hi_reset`, `_hi_purged_v7` | 25 mini-jeux **100 % locaux et hors ligne**. ⚠ **Flip 7** ne simule rien : c'est le TABLEAU DES SCORES d'une partie de cartes réelle (victoire à `FLIP7_GOAL`), donc une **donnée** et non un record — `spotifyplus_flip7` voyage dans la sauvegarde (clé `flip7`, REMPLACÉE à la restauration : unir deux listes de manches en inventerait) et s'exporte en CSV. `useImmersiveLock(true)` ; `useHidePlayerBar(true)` ; `useNoPinchZoom(active)` (Osu) — ⚠ écouteurs **non passifs** obligatoires. ⚠ `_doodle_hi_reset` = migration jouée **une seule fois**, l'effacer remet le record à 0. ⚠ Retirer un jeu ⇒ ajouter son id à `REMOVED_GAMES` (cumulatif) et **renommer** `HI_PURGE_LS` (`_v8`…), sinon la purge ne repart pas là où l'ancienne a déjà tourné |
...
| **Mot de Passe** | `GestionMdpPanel`, ambre | `spotifyplus_mdp_vault` | AES-GCM 256 + PBKDF2 600k. ⚠ La clé dérivée est CACHÉE en mémoire (`_mdpKeyCache`, 4 max, vidé au verrouillage/déconnexion) et `mdpEncrypt` REJOUE le sel connu : **ce qui doit rester neuf à chaque chiffrement, c'est l'IV, pas le sel**. Le blob chiffré contient `{entries, tags}` → suivi par les sauvegardes. Verrouillage auto 10 min. ⚠ Mot de passe oublié = irrécupérable |
...
| **Santé** (id `food`) | `NourriturePanel`, `#9bd93f` | `_food_profile`, `_food_days`, `_food_meals` (local seul), `_food_weights`, `_food_favs`, `_food_sport` | Calories (Open Food Facts + IA + photo de plat), macros, pesées. Chaque repas est ré-éditable (nom, catégorie, kcal, macros ; champ de macro vidé ⇒ la macro redevient INCONNUE, jamais 0). ⭐ Favoris : valeurs /100 g épinglées (id = `nom|marque`, jamais d'image), recherche LOCALE dédiée. 🏃 `FoodSportSection` : sport + durée + intensité → kcal brûlées (`foodSportKcal` = MET × poids × heures, `FOOD_SPORTS`/`FOOD_INTENS` ; `manual:true` = kcal corrigées à la main, plus jamais recalculées). ⚠ Le sport ne se soustrait PAS des repas : il RELÈVE l'objectif du jour (`targetEff = target + dayBurn`), `_food_days` reste le total avalé. ⚠ Un SEUL sélecteur de jour (chips sous le profil) commande la carte 🍽️ Nourriture ET la carte 🏃 Sport — elles parlent toujours de la même journée |
