...
| `WebApp` | Layout desktop (sidebar gauche + contenu) — barre du haut avec **sélecteur de vue** `view` ('scrapping' / 'artistes') : le contenu central bascule entre le feed (`ScrapingStatusPanel` + `FeedList`) et `ArtistsPanel`. **⚠ Plus de colonne de droite** : les sections Vos écoutes / Historique / Météo / Finance / To do / Mot de passe / Remember ont été déplacées **au-dessus du contenu**, dans un **bandeau horizontal** (`display:flex; flexWrap:wrap`) de cartes `SideCard` (chaque carte `flex:1 1 260px`, `maxHeight:360` + scroll interne). La sidebar gauche (`DateRangePanel` / `NextCallPanel` / `LogsPanel`) reste inchangée. **Desktop uniquement** — le mobile (`MobileApp`) garde ses onglets |
| `SideCard` | Carte d'une section du bandeau supérieur desktop (`{ label, color, children }`) : titre coloré + contenu, largeur fluide (`flex:1 1 260px`, `maxWidth:340`), `maxHeight:360` avec `overflowY:auto` pour qu'une section haute (Météo/Finance) ne pousse pas le feed trop bas |
