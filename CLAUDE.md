...
- **⚠ Un secret ne traverse JAMAIS un relais public sans accord explicite.** Deux cas : la clé Riot (l'API Riot n'ouvre pas le CORS) — `lolApiGet` tente d'abord le DIRECT avec `X-Riot-Token` (**`*.api.riotgames.com` doit rester dans `connect-src`**, sinon la CSP tue cet essai et la clé part toujours par les relais), puis les relais **uniquement** si `spotifyplus_lol_relay === 'on'` (case à cocher du ⚙ LoL, **absent = non**) ; et le token Clash Royale, qui passe par `proxy.royaleapi.dev` — relais imposé par Supercell (token lié à l'IP DU PROXY, donc inutilisable ailleurs). Steam / Google CSE / Alpha Vantage / TwelveData mettent aussi leur clé en query string, mais vont **directement** chez le propriétaire de l'API — c'est le protocole prévu, rien à changer.
...
32. **Des requêtes 404 fantômes vers `{expr}` au chargement à froid** : le *preload scanner* du navigateur lit la réponse par fragments réseau et interprète du JSX à cheval sur une coupure — `<img src={prev}>` devient un attribut HTML non quoté, d'où un GET de `/{prev}`. Les URL **changent à chaque chargement**, aucune n'entre dans le DOM, et le service worker ne les cache pas (`res.ok`). Bruit inévitable tant que l'app tient en un seul fichier : **ne pas chercher le bug dans le code**.

**Environnement d'agent**
33. **Le proxy réseau bloque de nombreux hôtes** (Google News, LinkedIn, Steam, CoinGecko, docs.google.com, YouTube, instagram…) → écrire un parsing défensif et **dire explicitement « non vérifiable côté agent »** plutôt que d'affirmer.
34. **unpkg est bloqué** : pour tester la page complète dans Chromium, servir React/Babel en local et bloquer le service worker (`serviceWorkers:'block'`), sinon il court-circuite les routes de test.
