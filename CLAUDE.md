...
| `TodoPanel` | **Section To do** (titre `TO DO` en violet `TODO_PURPLE = '#b18cff'`) — sidebar droite desktop **sous Finance** + **onglet dédié `todo`** mobile. Gestion de tâches persistées en `localStorage` (`spotifyplus_todos`, helpers `loadTodos`/`saveTodos`) classées par échéance via un **carrousel** : `TODO_BUCKETS` = Aujourd'hui / Dans la semaine / Dans le mois / Dans l'année / À faire un jour. Flèches ‹ › + points indicateurs pour changer de bucket affiché ; champ + bouton **+** (ou Entrée) ajoutent une tâche **dans le bucket courant** ; chaque tâche a des flèches ‹ › pour la **déplacer d'un cran d'échéance** (`move`) et une **×** rouge pour la supprimer. Chaque todo = `{ id, text, bucket }` |
