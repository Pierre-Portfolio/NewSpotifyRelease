...
| **Jeux** | `JeuxPanel`, `#39b2d1` | `spotifyplus_game_hi_*`, `tierlist-v1`, `_doodle_hi_reset` | 32 mini-jeux 100 % locaux, menu déroulant maison groupé par nombre de joueurs. `useImmersiveLock(true)` sur le module ; `useHidePlayerBar(true)` dans Doodle Jump masque `MobilePlayer`/`PlayerBar` (compteur module-level `_hidePlayer` + `usePlayerHidden`, affichage seul — le poll et l'auto-avance continuent). ⚠ `_doodle_hi_reset` est le drapeau d'une migration **jouée une seule fois** : l'effacer remet le record de Doodle Jump à 0 au prochain chargement |
