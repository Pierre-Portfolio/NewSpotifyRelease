...
- **⚠ Pas de build tool** : toute l'app est UN seul `<script type="text/babel" data-presets="react">` compilé au runtime — **`react` SEUL, jamais `env`** (il réécrivait tout en ES5 : 66 s de démarrage sur téléphone au lieu de 21). La moindre erreur de syntaxe fait échouer la compilation entière → `#root` vide → **écran tout noir**, sans message. **Après toute modif d'`index.html`, vérifier que la page monte avant de commit.**
