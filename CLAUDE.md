...
Constante module-level `APP_VERSION` (**actuellement `'2.6.5'`**, commit ≈265), format `MAJ.MIN.U` = **le nombre de commits du projet avec des points séparant le dernier chiffre (patch), l'avant-dernier (minor) et tout le reste (major)** : `patch = N % 10`, `minor = floor(N/10) % 10`, `major = floor(N/100)`. Exemples : 265 commits → `2.6.5`, 270 → `2.7.0`, 300 → `3.0.0`, 1001 → `10.0.1`, 1234 → `12.3.4`. Affichée en gris sous le bouton « 🗑 Purger les écoutes » de `VosEcoutesPanel` (« Version 2.6.5 »). **⚠ Le compteur suit l'historique du PROJET (≈265), pas `git rev-list --count HEAD` de ce dépôt-fork qui est bien plus bas (~65)** — incrémenter à la main en suivant le compteur projet.
