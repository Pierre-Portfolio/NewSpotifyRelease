...
| **À propos** | `VosEcoutesPanel` | — | Paramétrage · Lancement automatique · **Autorisations** · Exporter · Sauvegardes cloud · Restaurer · Réinit quota · Purger · Déconnecter |

**Autorisations externes** : `AUTH_PERMS` = **la table unique** (Drive, Drive lecture seule, YouTube, Dropbox, pCloud, notifications, géoloc, stockage persistant) ; `AuthPermsPanel` la rend en entier dans À propos, filtrée par `only` dans Météo / Alertes / Musique (tableaux `*_PERMS` **constants**, sinon l'effet reboucle). ⚠ **Aucun « tout autoriser » n'est possible** : une popup OAuth doit partir du même tick qu'un clic, et Google refuse `youtube` + `drive.file` ensemble → un bouton par ligne. États navigateur relus au montage et sur `visibilitychange`.
