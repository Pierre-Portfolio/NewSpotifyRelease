...
| **Bon Plan** | `ComparePanel`, bleu-indigo | `spotifyplus_cmp_watch` | Ma liste · Promos (CheapShark — ⚠ `/deals` renvoie **une ligne par boutique** : regroupées par `gameID`, meilleure offre seule, vignette → tableau site/prix ; ⚠ Deluxe et Ultimate sont deux `gameID` **différents** → 2e passe `cmpMergeEditions`, Ultimate en tête, le jeu de base jamais absorbé) · Bibliothèque Steam · Recherche jeux · Tous produits (IA + recherche web) |
