...
### `APP_VERSION` (actuellement `'6.6.5'`)
...
`TOGGLE_MODULES` = modules basculables `{id, label, icon, color, desktop, mobile}` (Actu/Météo/Finance/Frigo Massy/Jeux/Bon Plan/TV Time/Musique/Note/Santé/Vêtement/To do/Maps/Mot de passe/Remember/Stats ; Scrapping & À propos non basculables). `loadModuleToggles()`/`saveModuleToggles()` (localStorage `spotifyplus_modules`), `moduleEnabled(toggles, id)` (absent = activé, seul `false` désactive). UI = toggles dans « ⚙️ PARAMÉTRAGE » (À propos). Onglet actif désactivé → retour à `home`. Inclus dans buildBackup.

---

## Sauvegarde / Restauration

`buildBackup()` complet = `{ stats, artists, music (table tracks), todos, todos_done, rappels, rappels_time, remembers, tvtime (+ YouTube), tmdb_key, yt_key, maps, finance (historique prix), food (profil + totaux kcal/jour, PAS la composition des repas), bonplan (jeux suivis + prix cibles), mdp_vault (chiffré), notes (notes — les chiffrées restent chiffrées), modules, listens }`. **Caches re-fetchables exclus** : actu, indices, dependabot, speedrun, lol, yt_subs, actifs, tv eps. **Exclu volontairement** : composition des repas Nourriture (`spotifyplus_food_meals`).
...
**Vêtement** (`VetementPanel`, orange `VETEMENT_COLOR='#f59a2e'`, entre TV Time et Stats) : module **vide pour le moment** (placeholder « contenu à venir »). Basculable (`TOGGLE_MODULES` id `vetement`), présent partout comme un module normal (onglet desktop `navItems`, menu mobile `allOverflowTabs` + bloc de rendu `MobileApp`, `SECTION`, chip de la grille `HUB_FEATURES`). PAS sur l'accueil connecté (`HUB_CARDS`). PAS dans buildBackup (rien à sauvegarder pour l'instant).
...
**Jeux** (`JeuxPanel`, bleu-cyan `GAMES_COLOR='#39b2d1'`, entre Finance et Maps) : ludothèque 100% locale (zéro réseau). Un seul onglet avec un **menu déroulant maison en haut à droite aligné avec le titre « JEUX »** (`selfHeading:true` dans `SECTION.games`) pour choisir parmi **14 mini-jeux**. ⚠ Ce menu remplace la selectbox `<select>` native (dont la liste OS remplissait tout l'écran, non ergonomique) : bouton (label du jeu courant + chevron ▾) qui ouvre un **popup compact** (gabarit du menu ⋯ des modules — `C.surfaceHigh`, bordure, overlay `position:fixed inset:0` pour fermer au clic extérieur, `maxHeight:60vh` scrollable, item actif surligné en `GAMES_COLOR`), state local `open` (`GAMES`, ordre ALPHABÉTIQUE, chiffres en tête) : 2048, Bataille navale, Démineur, Flappy Bird, Go (2 joueurs), Guitar Hero, Inposteur, Memory, Morpion (2 joueurs), Motus, Osu, Puissance 4 (2 joueurs), Snake, TierList. Dernier jeu ouvert mémorisé (`spotifyplus_games_last`). `GameHost` = `switch` qui rend le composant du jeu sélectionné (repli `GameComingSoon`). Chaque jeu est un composant autonome, 100% local : **2048** (`Game2048`, fusion de tuiles, clavier+swipe via `useSwipe`), **Bataille navale** (`GameBataille`, contre l'ordinateur, 2 grilles 8×8, placement auto `bnPlace`, IA qui cible les voisins après un touché), **Démineur** (`GameDemineur`, 9×9/10 mines, 1re case sûre, mode 🚩), **Flappy Bird** (`GameFlappy`, canvas + `requestAnimationFrame`, tap/Espace), **Go** (`GameGo`, 9×9 SVG, capture par groupe `goGroup`/`goNeighbors` + ko simple + passe), **Guitar Hero** (`GameGuitarHero`, rythme : notes qui tombent sur 4 couloirs, `GH_LANE_KEYS` D/F/J/K + boutons tactiles, zone de frappe `GH_HIT_Y`/`GH_HIT_WIN`, boucle 30 fps via `applyNotes` (ref+state = source unique anti-closure), **manche 60 s** `GH_ROUND` + **5 niveaux** `GH_LEVELS` réglant vitesse/cadence, niveau persistant `spotifyplus_game_gh_level`, combo, note ratée ou frappe à vide → combo cassé, record `gameHi('guitar')`), **Inposteur** (`GameInposteur`, pass-and-play type Undercover, `INPOSTEUR_PAIRS`, phases setup→reveal→discuss→result), **Memory** (`GameMemory`, 8 paires d'emojis 4×4, records = nombre de coups mini), **Morpion** (`GameMorpion`, `MORPION_WINS` ; **variante 3 pions max** `MORPION_MAX=3` : state `order` = pions dans l'ordre de pose, au 4e pion d'un joueur son plus ancien est retiré → **jamais d'égalité, il y a toujours un vainqueur** ; le pion condamné est grisé/bordure orange en avertissement, plus de `draw`), **Motus** (`GameMotus`, `MOTUS_WORDS` 6 lettres, `motusEval`, clavier AZERTY à l'écran + physique), **Osu** (`GameOsu`, cercles DOM à cliquer, combo, **manche de 60 s** `OSU_ROUND`, **5 niveaux de difficulté** `OSU_LEVELS` réglant cadence d'apparition `spawn`/durée de vie `life`/taille `size` — niveau persistant `spotifyplus_game_osu_level`, sélecteur verrouillé pendant la manche, bonus de score `60+level*40` par cercle, anim `osuAppr` calée sur `life`), **Puissance 4** (`GamePuissance4`, `p4Win`), **Snake** (`GameSnake`, `setInterval` + refs anti-closure, dpad), **TierList** (`GameTierList`, S/A/B/C/D par tap, persistant `spotifyplus_game_tierlist`). Records dans `spotifyplus_game_hi_<id>` (`gameHi`/`setGameHi`). Helpers partagés : `gameBtn`, `GameWrap` (conseil + barre score/reset + contenu centré), `useSwipe`. Les jeux 2 joueurs se jouent sur le même téléphone. ⚠ Une valeur de prop `bar={<>…</>}` avec plusieurs `{expr && <button/>}` doit fermer CHAQUE `{}` (un `}` manquant → écran noir Babel). Basculable (`TOGGLE_MODULES` id `games`) ; **PAS de carte sur l'accueil connecté** (`HubHome`) ; **accessible depuis la page de CONNEXION** (chip « Jeux » de `HUB_FEATURES` avec `toolKey='games'` → panneau superposé, comme Note/Calculatrice, tout est local). **PAS dans buildBackup.** ⚠ Nouvel onglet mobile → bloc de rendu `{tab === 'games' && <JeuxPanel/>}` présent dans `MobileApp`.
