...
| `GestionMdpPanel` | **Section Gestion MDP** (titre `GESTION MDP` en violet `MDP_PURPLE = '#a78bfa'`) — sidebar droite desktop **sous To do** + **onglet `mdp`** mobile. Petit gestionnaire d'identifiants persistés en `localStorage` (`spotifyplus_mdp`, helpers `loadMdp`/`saveMdp`). Chaque entrée = `{ id, identifiant, mdp, url, comment }`. Formulaire d'ajout (4 champs + bouton **+ Ajouter**), liste avec mot de passe **masqué** (toggle 👁/🙈) + bouton copier 📋, lien ↗ URL, commentaire, **×** rouge pour supprimer. ⚠ **Stocké en clair** (pas de chiffrement, pas de backend) → usage perso, avertissement affiché |
