...
Constante module-level `APP_VERSION` (ex. `'1.18.6'`), format `1.MM.U` où `MM.U` = **nombre de commits du dépôt avec un point inséré avant le dernier chiffre** (186 commits → `1.18.6`, 190 → `1.19.0`, 200 → `1.20.0`). Affichée en gris sous le bouton « 🗑 Purger les écoutes » de `VosEcoutesPanel` (« Version 1.18.5 »).
