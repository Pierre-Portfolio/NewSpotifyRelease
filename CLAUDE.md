...
**IA par module** (bouton 🤖 de la même ligne, `ModuleAiModal`) : `AI_MODULES` = `{id: {label, why}}` (`why` = le « pourquoi de l'IA ici » affiché **avant** la config) — **la table qui décide** ; un module absent a son 🤖 **grisé**. Stockage `spotifyplus_module_ai` = `{modId:{provId:false}}`, **absent ⇒ autorisé**. `aiChain(needVision, modId)` filtre la chaîne, donc décocher **raccourcit les replis**, ça ne change ni la requête ni le parsing (`geminiExtract`) : changer d'IA ne peut pas casser un module. Chaîne vide alors que des clés existent → `AI_OFF_MSG` (≠ `NO_KEY`). ⚠ Cache module-level `_modAiCache` → **`moduleAiInvalidate()` après toute écriture hors `saveModuleAi`** (c'est ce que fait `applyBackup`).
...
**`buildBackup()`** (synchrone) → `{ stats, artists, music, todos, todos_done, rappels, rappels_time, remembers, tvtime, tmdb_key, yt_key, maps, finance, food, bonplan, mdp_vault, notes, notes_enc_check, modules, home_modules, module_ai, listens, frigo, vetement, histoire, emploi, sport, revente, collection, gemini_key, groq_key, cerebras_key, openrouter_key, mistral_key, cohere_key, zai_key, cloudflare_key/account, gcse_key, gcse_cx, github_token, chess_user, speedrun_user, lol, games, api_calls, prefs }`.
...
| **Actu** | `ActuPanel`, bleu-indigo | caches 30 min | Collapses en ordre alphabétique. Sous-sections : Actualité (9), Cartographie (iframes `MapFrame`), Fuites de données, Vidéos YouTube (OAuth séparé ; lecture dans le **`YtFullscreenPlayer` de TV Time**, file = tout l'onglet courant. Vue à **`ACTU_SEEN_RATIO`** (50 %) ou à la fin → `ytSeenMarkAnywhere` la classe dans TV Time + `ytWlDropSeen` la sort de la playlist. ⚠ `ytSeenMarkAnywhere` cherche d'abord la chaîne qui CONTIENT la vidéo, puis à défaut celle dont l'id vaut `v.channelId` — les vidéos d'Actu sont trop récentes pour être déjà dans la liste de la chaîne suivie ; l'id coché hors liste est un « orphelin », compté par `ytOrphanSeen`, d'où le garde-fou `ytItemAcceptsVideo` sur les Shorts). Priorité `data/actu.json`, repli proxies CORS |
| **Alertes** | `RappelPanel`, `#6f7bf5` | `spotifyplus_rappels`, `_rappels_time`, `_remember_custom`, `_speedrun_todo_added` | Rubriques fixes + rappels persos + sections auto (Frigo, Steam). Une file speedrun non vide pose une tâche « Aujourd'hui » du To do (`syncSpeedrunToTodo`, marqueur `srKey` + 1 création/jour ; le compte y est réécrit, file vidée ⇒ tâche et marqueur retirés). `RAPPEL_RECUR` = récurrences en mois ; `rappelNextDate` calcule la prochaine occurrence (la date stockée reste celle d'origine). Calendrier mensuel 🗓 |
...
| **Bon Plan** | `ComparePanel`, bleu-indigo | `spotifyplus_cmp_watch` | Ma liste · Promos (**3 sources** versées dans la même liste : CheapShark, `cmpSteamDeals` (`featuredcategories` via proxies CORS) et `cmpEpicDeals` (`freeGamesPromotions`, seul endpoint Epic keyless — jeu offert de la semaine ; ⚠ tester la fenêtre `startDate`/`endDate`, une promo finie reste dans la réponse). ⚠ CheapShark est en **USD converti**, Steam et Epic en **euros réels** → chaque offre porte `__eur` et `px(v, __eur)` décide de convertir ou non ; le tri se fait sur `cmpEurVal`. Regroupement par **titre normalisé** (`cmpTitleKey`), seul identifiant commun aux 3 sources ; ☆ suivi réservé aux jeux ayant un `gameID` CheapShark. ⚠ `/deals` renvoie **une ligne par boutique** : meilleure offre seule, vignette → tableau site/prix ; ⚠ Deluxe et Ultimate sont deux `gameID` **différents** → 2e passe `cmpMergeEditions`, Ultimate en tête, le jeu de base jamais absorbé) · Bibliothèque Steam · Recherche jeux · Tous produits (IA + recherche web) |
...
| **Finance** | `FinancePanel`, cyan | `spotifyplus_finance_history` | État partagé module-level `_finance` (survit au démontage). 5 modes : Light / Full / Calculatrice / BullRun / Mes Actifs. Chargement **lazy par mode**. Relevé quotidien auto (`finAutoDue`) : charge Light **puis** Full, et échantillonne (`finHistSnapshot`) tout le Light + les lignes Full d'INDICES et de STOCK PICKING — le reste du Full n'enregistre rien |
| **Frigo** | `FrigoPanel`, `#47a8de` | `spotifyplus_frigo` | 3 compartiments + péremption + assistant IA (photo → ingrédients, recettes) |
| **Histoire** | `HistoirePanel`, `#41b0d8` | `spotifyplus_histoire` | Ville + photos → fiche d'œuvre par IA (recherche web ancrée) + illustrations Google Custom Search. Fil de questions (`chat` DANS la fiche, donc sauvegardé) sous « À savoir » : `histChatPrompt` rappelle TOUJOURS ville + œuvre avant la question |
| **Jeux** | `JeuxPanel`, `#39b2d1` | `spotifyplus_game_hi_*`, `tierlist-v1`, `_doodle_hi_reset`, `_hi_purged_v2` | 30 mini-jeux 100 % locaux + **3D Pinball**, seule entrée qui n'est PAS un jeu local : un `<a target="_blank">` vers le portage web d'origine (donc aucune origine à ajouter à la CSP — une iframe, elle, l'imposerait). Menu déroulant maison groupé par nombre de joueurs. `useImmersiveLock(true)` sur le module ; `useHidePlayerBar(true)` dans Doodle Jump masque `MobilePlayer`/`PlayerBar` (compteur module-level `_hidePlayer` + `usePlayerHidden`, affichage seul — le poll et l'auto-avance continuent). ⚠ `_doodle_hi_reset` est le drapeau d'une migration **jouée une seule fois** : l'effacer remet le record de Doodle Jump à 0 au prochain chargement |
...
| **Météo** | `MeteoPanel`, bleu | `spotifyplus_meteo_rain_notified` | Open-Meteo sans clé, 5 lieux (dont position actuelle), Light 3 j / Full 7 j. **Alerte pluie** `meteoRainCheck()` (effet du store, T+20 s puis 15 min) : notifie une pluie à venir dans les 2 h, **1× par ÉPISODE** — la clé mémorisée est l'heure de DÉBUT des heures pluvieuses contiguës (mémoriser la prochaine heure pluvieuse re-notifierait une pluie déjà commencée). Géoloc **seulement si déjà autorisée** (aucun prompt au boot), sinon `METEO_LOCS[0]` |
...
| **TV Time** | `TvTimePanel`, ambre | `spotifyplus_tvtime`, `_tvtime_eps`, `_yt_progress` | Séries/films (TMDB), chaînes YouTube, livres (Google Books + Open Library). Source de vérité = `tvWatchedSet` — ⚠ un item hérité (sans `watchedSet`) le RE-DÉRIVE de `watchedEps` sur `seasons`, réécrit à chaque refresh : un compteur ≥ total avalait chaque nouvel épisode (coché tout seul à sa sortie), d'où `tvFreezeWatchedSet` qui le fige AVANT tout refresh. Lecteur plein écran `YtFullscreenPlayer` : la **fin** d'une vidéo de la playlist la valide, et sa position est mémorisée dans `_yt_progress` (⚠ **l'API YouTube ne donne AUCUNE position de lecture** — seul ce lecteur en connaît une ; hors sauvegarde). Synchro quotidienne `tvSyncDaily` reprenable |
| **Vêtement** | `VetementPanel`, `#f59a2e` | `spotifyplus_vetements`, `_vetement_prix`, `_vetement_img` | 35 types en sous-catégories, fiches + marques rangées en sections (`brand.groupe` = clé de `VET_GROUPS`, `''` = « Possédé » ; glisser-déposer ou puce du formulaire — mais TOUTES restent proposées dans la fiche d'un vêtement). **Photos sur Google Drive**, jamais en local. Barre de filtres + recherche par image (IA). Deux réglages d'AFFICHAGE persistés au titre du module : 💶 masque les prix, 🖼️ remplace les fiches par une grille de photos sous le collapse de sous-catégorie (`'on'` = images, absent = liste) |
...
**IA** : **8 fournisseurs en chaîne de replis**, façade unique **`aiGenerate(modId, parts, opts)`** (elle pose `opts.module` puis délègue à `geminiGenerate`) — **c'est le seul point d'entrée des modules**. Ordre : `gemini → groq → openrouter → mistral → cerebras → zai → cohere → cloudflare`, filtré par les IA cochées pour `modId` (cf. **IA par module**). `AI_PROVIDERS` porte `vision` (4 seulement) et `ls2`/`url` fonction (Cloudflare = 2 champs). ⚠ Une requête avec image **saute** les fournisseurs sans vision (sinon réponse inventée). ⚠ `opts.tools` (recherche web ancrée) **n'existe que chez Gemini** → sinon `NO_GROUNDING` (≠ `NO_KEY`, que les appelants testent pour abandonner) et l'appelant rejoue sans outil. `aiAnyKey(needVision, modId)` garde l'UI, `aiLastProvider()` nomme celui qui a servi. ⚠ **Le modèle Gemini n'est PAS figé** : `GEMINI_MODELS` est essayée dans l'ordre, un **404, un 403 ou un 5xx passe au suivant** (un 5xx est d'abord **rejoué une fois** sur le même modèle : c'est une panne Google, fréquente sur le résumé de vidéo, et le message final le dit au lieu d'accuser la clé), le gagnant est mémorisé pour la session et lu par `geminiModel()` — Google retire ses modèles (`gemini-2.0-flash` coupé le 01/06/2026 ⇒ toute l'IA du Hub en « Erreur IA (HTTP 404) », clé pourtant valide ; une clé morte, c'est un **400**). N'y mettre que des **Flash** : les Pro sont payants depuis avril 2026.
