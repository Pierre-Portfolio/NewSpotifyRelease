...
| **Jeux** | `JeuxPanel`, `#39b2d1` | `spotifyplus_game_hi_*`, `tierlist-v1`, `_doodle_hi_reset` | 28 mini-jeux 100 % locaux, menu déroulant maison groupé par nombre de joueurs. `useImmersiveLock(true)` sur le module ; `useHidePlayerBar(true)` dans Doodle Jump masque `MobilePlayer`/`PlayerBar` (compteur module-level `_hidePlayer` + `usePlayerHidden`, affichage seul — le poll et l'auto-avance continuent). ⚠ `_doodle_hi_reset` est le drapeau d'une migration **jouée une seule fois** : l'effacer remet le record de Doodle Jump à 0 au prochain chargement |
...
| **TV Time** | `TvTimePanel`, ambre | `spotifyplus_tvtime`, `_tvtime_eps`, `_yt_progress` | Séries/films (TMDB), chaînes YouTube, livres (Google Books + Open Library). Source de vérité = `tvWatchedSet`. Lecteur plein écran `YtFullscreenPlayer` : la **fin** d'une vidéo de la playlist la valide, et sa position est mémorisée dans `_yt_progress` (⚠ **l'API YouTube ne donne AUCUNE position de lecture** — seul ce lecteur en connaît une ; hors sauvegarde). Synchro quotidienne `tvSyncDaily` reprenable |
