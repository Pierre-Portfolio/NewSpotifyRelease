...
- **⚠️ CSP** (`<meta http-equiv="Content-Security-Policy">` dans le `<head>`) : `connect-src` limité à `'self'` + `api.spotify.com` + `accounts.spotify.com` + `api.open-meteo.com` (météo) + `nominatim.openstreetmap.org` + `api.bigdatacloud.net` (reverse-geocoding du nom de ville, Nominatim en priorité + BigDataCloud en fallback) + `api.coingecko.com` + `api.frankfurter.app` + `api.gold-api.com` + `stooq.com` (Finance : crypto, EUR/USD, or, pétrole) — même en cas d'XSS, le token ne peut pas être exfiltré vers un domaine attaquant. `'unsafe-inline'`/`'unsafe-eval'` requis par Babel standalone, `'wasm-unsafe-eval'` par sql.js. **Toute nouvelle origine (script, image, fetch) doit être ajoutée à la CSP sinon elle est bloquée par le navigateur.**
