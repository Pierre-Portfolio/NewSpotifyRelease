...
restoreBackup(data)            // restaure une sauvegarde buildBackup : UPSERT prudent artists_scraped (dates de scan jamais régressées, cumuls au MAX), stats (reprise intégrale si appareil vierge, sinon fusion des cumuls), todos/rappels/remembers/tvtime REMPLACÉS. Bouton « ↥ Restaurer mes données » dans VosEcoutesPanel (input file JSON + confirm)
