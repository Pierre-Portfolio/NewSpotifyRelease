...
**GitHub/Speedrun stats** (`GithubStatsSection`/`SpeedrunStatsSection`) : réutilisent les accès de Veilles Techno. `ghFetchProfile` (1 GraphQL viewer, streak), `srFetchStats` (personal-bests). PAS dans buildBackup. Ordre GitHub : Dépôts → Abonnés → Étoiles reçues → Dépôt star → Contributions → Commits → Forks → Langage → Série active (dernier).
