...
- **⚠ CSP** (`<meta>` dans `<head>`) : `connect-src` limité à `'self'` + les origines réellement utilisées (api.spotify.com, accounts.spotify.com, api.themoviedb.org, www.googleapis.com, api.open-meteo.com + geocoding, api.bigdatacloud.net/photon.komoot.io/nominatim, api.coingecko.com, api.frankfurter.app, api.gold-api.com, api.twelvedata.com, stooq.com, proxies CORS codetabs/cors.eu.org/allorigins, api.github.com, www.speedrun.com, hn.algolia.com, hacker-news.firebaseio.com, haveibeenpwned.com, www.cheapshark.com, ledenicheur.fr, ddragon.leagueoflegends.com, api.dropboxapi.com/content.dropboxapi.com, tile.openstreetmap.org). `img-src` inclut `https:` + i.ytimg.com + *.ggpht.com/*.googleusercontent.com + image.tmdb.org + ddragon. `'unsafe-inline'`/`'unsafe-eval'` requis par Babel, `'wasm-unsafe-eval'` par sql.js. **Toute nouvelle origine (fetch/script/image) DOIT être ajoutée à la CSP sinon le navigateur la bloque.**
...
**Actu** (`ActuPanel`, bleu-indigo) : 1) 🔓 Fuites (10, bonjourlafuite → repli HIBP) · 2) 📰 Actualité (6 sous-collapses : Tendances Google Trends, Monde, Bourse&crypto, Presse FR IA/Science + Hacker News, Jeux, Insolite) · 3) ▶ Vidéos YouTube abonnements (`ActuYoutubeSection`, OAuth scope `youtube`, lazyload, bouton 🕒 → playlist « À regarder plus tard (Hub) »). Priorité `data/actu.json`, repli proxies CORS. Vignettes via `actuThumb` (photo article ou favicon). Cache 30 min. **PAS dans buildBackup.**
...
**League of Legends** (`LolStatsSection` dans 🎮 JEUX de Stats) : API Riot (clé perso `spotifyplus_lol_key`, **Personal API Key permanente** recommandée). Riot sans CORS → proxies CORS (`lolApiGet`). Data Dragon direct (`ddragon`, sans clé). `lolFetchProfile` (account→summoner→league→mastery), `lolFetchLive` (spectator-v5), `lolFetchRanks` (rangs des 10 joueurs, cache 15 min). Config `spotifyplus_lol_cfg` `{riotId, platform}`.
