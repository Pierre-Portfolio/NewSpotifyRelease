...
- **« Stockage local saturé » ?** À propos → 📦 *Limite data* montre où part la place : espace utilisé, espace **encore libre** (mesuré sur ton navigateur, pas supposé), puis **le poids de chaque module** — 📦 son stockage local *et* 🗄️ sa base (SQLite pour Musique, pièces jointes pour Note). Touche une ligne pour voir ses clés une par une et repérer la plus grosse.
