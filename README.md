...
- **Matières premières** : Pétrole (WTI), Sucre et Blé via **[Alpha Vantage](https://www.alphavantage.co/)** (endpoints commodités dédiés `WTI`/`SUGAR`/`WHEAT`, CORS natif, vrais prix + variation), Or/Argent via **[gold-api.com](https://gold-api.com/)**
...
- **Chargement uniquement à l'ouverture de Finance** : les cours ne se chargent **plus au login** mais seulement lorsqu'on ouvre la section/onglet Finance (valeurs Light en priorité) — évite de consommer le quota Alpha Vantage (~25 req/jour) si on n'ouvre jamais Finance. Un **indicateur de chargement** clair s'affiche pendant la récupération (les appels boursiers Twelve Data sont espacés de ~8 s pour respecter la limite gratuite)
