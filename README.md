...
### Actu
Agrégateur **sans backend ni clé** (cache 30 min, ↻ Actualiser). Sources pré-récupérées par une **GitHub Action** (`data/actu.json`, proxies CORS en repli). Contient : **🔓 Fuites de données** (bonjourlafuite → repli Have I Been Pwned), **📰 Actualité** (Tendances Google, Monde, Bourse & crypto, Presse FR IA/Science + Hacker News, Jeux, Insolite) et **▶️ Vidéos YouTube · Abonnements** (nécessite de connecter Google en lecture seule ; bouton « 🕒 Plus tard » ajoute la vidéo dans une playlist YouTube dédiée). Chaque actu a sa vignette.
