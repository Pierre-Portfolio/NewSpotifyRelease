...
### Actu
Agrégateur **sans backend ni clé** (cache 30 min, ↻ Actualiser). Sources pré-récupérées par une **GitHub Action** (`data/actu.json`, proxies CORS en repli). Contient : **🔓 Fuites de données** (bonjourlafuite → repli Have I Been Pwned), **📰 Actualité** (Tendances Google, Monde, Bourse & crypto, Presse FR IA/Science + Hacker News, Jeux, Insolite), **🚇 Réseaux de Transport Paris** (carte interactive des transports franciliens intégrée, avec lien plein écran) et **▶️ Vidéos YouTube · Abonnements** (nécessite de connecter Google en lecture seule ; bouton « 🕒 Plus tard » ajoute la vidéo dans une playlist YouTube dédiée). Chaque actu a sa vignette : la **photo de l'article** (résolue côté GitHub Action via l'og:image de la page), sinon le favicon de la source en dernier recours.
...
- **Export / Import** JSON, complet **ou** partiel (11 exports séparés) — fusion prudente à la restauration (dates de scan jamais régressées, TV Time/Finance fusionnés…)
