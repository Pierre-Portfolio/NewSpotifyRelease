...
- **Filtre** par type : Tous / Singles / Albums / Spotify (titres « Découvertes de la semaine »)
- **Tri par défaut : par artiste (A→Z)** — modifiable en Date d'ajout / Date de sortie
...
- **🎮 Jeux** (tout en bas) — sous-section **⚔️ League of Legends** via l'**API officielle Riot Games** (gratuite ; clé à générer sur developer.riotgames.com avec son compte Riot — **recommandé : Personal API Key permanente via « Register Product »**, elle n'expire jamais, contrairement à la Development Key 24 h — saisie dans l'UI et stockée en localStorage comme les clés TMDB/YouTube) : **profil** (Riot ID, niveau, icône d'invocateur), **rangs classés** Solo/Duo et Flexible (tier + division + LP, victoires/défaites, % de victoires), **top 3 maîtrises de champions**, et **partie en cours** (bandeau 🔴 EN PARTIE via Spectator : mode de jeu, durée, champion joué, et **le détail de chaque joueur des deux équipes — pseudo, champion, élo Solo/Duo et Flex, % de victoires, nombre de games** — rangs mis en cache 15 min par joueur). Noms/icônes de champions via **Data Dragon** (CDN public Riot, sans clé). Riot n'ayant pas de CORS, les requêtes passent par les proxies CORS keyless déjà utilisés par Finance/Actu ; profil caché 1 h, partie en cours toujours re-vérifiée
...
- **⚙️ Paramétrage des modules** : section **repliable** avec un **interrupteur par module** (Historique, Artistes, Actu, Météo, Finance, Bon Plan, TV Time, To do, Maps, Mot de passe, Remember, Stats) — activer/désactiver un module **fait apparaître/disparaître son onglet** en direct (desktop et mobile). Le réglage est inclus dans l'export/import (et la sync Dropbox/Drive)
...
- **Numéro de version** affiché en gris sous le bouton « Purger les écoutes » **et en badge sur la page de connexion** (à la place de « Zéro backend » ; actuellement `Version 5.9.4`) — basé sur le nombre de commits du projet (format `MAJ.MIN.U` : derniers chiffres = patch/minor, le reste = major ; ex. 278 commits → `2.7.8`, 1001 → `10.0.1`)

### Actu
- Section **Actu** dédiée (titre en bleu-indigo), **entre Artistes et Météo** — onglet en haut sur desktop, dans le menu « ⋯ » sur mobile. Agrégateur d'actualités **sans backend ni clé**, cache local (30 min) + bouton **↻ Actualiser**. **Chaque actu a sa vignette** (photo de l'article si le flux la fournit, sinon favicon du site source) — sauf les Tendances du jour, volontairement sans image. Les sources (presse, tendances, fuites) sont **pré-récupérées côté serveur par une GitHub Action** (toutes les 2 h → `data/actu.json`, lu directement par l'app) — les proxies CORS publics, peu fiables, ne servent plus que de repli
...
- **🎮 Jeux vidéo** : recherche d'un jeu → comparaison des prix sur **~20 boutiques PC** (Steam, GOG, Epic, Fanatical…) via l'API [CheapShark](https://www.cheapshark.com/) (gratuite, **sans clé**) — prix trié du moins cher au plus cher, % de réduction, prix barré, lien direct vers l'offre. Prix convertis **≈ en euros** (taux BCE du jour)
- **🔎 Tous produits** : recherche sur [leDénicheur](https://ledenicheur.fr/) — ⚠ le site (groupe Prisjakt/PriceSpy) **n'a pas d'API publique ni de clé** : l'app analyse la page de recherche en best-effort (via proxies CORS) et affiche dans tous les cas un **lien direct vers la recherche sur le site**
...
- **Heure optionnelle** : pour chaque rappel (fixe ou perso), un champ **heure** apparaît dès qu'une date est renseignée — pratique pour un rendez-vous à une heure précise (l'heure s'affiche à côté de la date dans la vue « Dates à venir » et dans la tâche ajoutée au To do). Dates **et heures** sont incluses dans l'export/import 🔔 Remember et la sauvegarde Dropbox/Drive
