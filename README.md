...
### Jeux
🎮 **Ludothèque 100 % locale** (aucune connexion, aucun réseau) : un seul onglet avec un **menu déroulant en haut à droite** (aligné avec le titre) pour choisir parmi **15 mini-jeux** — **2048, Bataille navale, Démineur, Flappy Bird, Go (à 2 joueurs), Guitar Hero, Inposteur, Memory, Morpion (à 2 joueurs), Motus, Osu, Puissance 4 (à 2 joueurs), Snake, TierList, UnderCoverDraw (3 à 8 joueurs)**. La Bataille navale se joue **à 2 sur le même téléphone**, chacun **plaçant lui-même ses navires** case par case (touche une case pour poser le navire, 🔄 pour le tourner, 🎲 pour un placement aléatoire) ; les autres jeux à 2 joueurs (Go, Inposteur, Morpion, Puissance 4) se jouent aussi **sur le même téléphone**. Le **Morpion** utilise la variante « 3 pions max » : chaque joueur ne garde que ses 3 derniers symboles (le plus ancien, grisé, disparaît quand on en pose un 4ᵉ) — **il y a donc toujours un vainqueur, jamais d'égalité**. L'**Osu** propose une manche de **60 s** et **5 niveaux de difficulté** (du Très facile à l'Extrême : plus c'est dur, plus les cercles apparaissent vite, sont petits et disparaissent tôt). Le **Guitar Hero** fait tomber des notes sur 4 couloirs à frapper au bon moment (touches D/F/J/K ou boutons tactiles), avec lui aussi une manche de **60 s** et **5 niveaux de difficulté** (la vitesse et la fréquence des notes montent d'un cran à chaque niveau). **UnderCoverDraw** est le **portage du projet perso** du même nom : jeu de société **dessin & déduction** façon Undercover en « passe-l'appareil » (3 à 8 joueurs, avec vos prénoms) — la majorité reçoit un mot, l'imposteur un mot **au dessin quasi identique** (base de **344 couples piégeux** en 20 catégories) ; chacun dessine son mot en croquis minimaliste sur un vrai tableau, impossible de savoir si l'on est l'imposteur avant le vote, puis l'écran de résultat révèle mot commun VS mot imposteur et le rôle de chacun. La **TierList** est désormais la **version complète du projet perso LocalTierList** : une vraie tier list à **images** — tu uploades tes images (redimensionnées et compressées en local, rien ne quitte l'appareil), puis tu les **glisses dans les rangs au doigt ou à la souris** (drag & drop tactile avec auto-scroll près des bords) ; rangs entièrement personnalisables en mode édition (nom, couleur, ajout/suppression — les images d'un rang supprimé retournent dans la réserve), et la sauvegarde est **partagée avec l'app LocalTierList d'origine** (même stockage local). Le **Motus** est désormais la **version complète du projet perso** : choix de la longueur du mot (**6 à 9 lettres**) et de la difficulté (**5 niveaux**, gradués par fréquence d'usage réelle), **850 mots à deviner**, propositions vérifiées contre un **dictionnaire de ~125 000 mots français** (mot inconnu → secousse), première lettre offerte, code couleur Motus authentique (🟥 carré rouge bien placé · 🟡 rond jaune mal placé · 🟦 bleu absent), gestion des lettres en double et révélation animée case par case. Chaque jeu est jouable au clavier **et** au tactile (glissé, pavé directionnel, clavier AZERTY à l'écran pour Motus). Les records personnels sont mémorisés, ainsi que le dernier jeu ouvert. Accessible aussi **dès la page de connexion** (tuile 🎮 Jeux de la grille de modules, sans connexion Spotify).

### Vêtement
👕 **Garde-robe 100 % locale** en deux temps : le collapse **Inventaire** compte combien tu as de chaque type de vêtement (T-shirt, Chaussettes, Pantalon…) avec des boutons + / −. Chaque **+1 crée automatiquement une fiche** dans le second collapse **Mes vêtements**, qui reste **vide au départ** et se remplit tout seul : un **sous-collapse par type** présent, contenant **une fiche par vêtement** (nom, photo, état, lieu). Supprimer une fiche (« je le jette ») diminue aussi l'inventaire d'une unité. Activable/désactivable dans le Paramétrage comme les autres modules.
