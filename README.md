...
### Jeux
🎮 **Ludothèque 100 % locale** (aucune connexion, aucun réseau) : un seul onglet avec un **menu déroulant en haut à droite** (aligné avec le titre) pour choisir parmi **13 mini-jeux** — **2048, Bataille navale, Démineur, Flappy Bird, Go (à 2 joueurs), Inposteur, Memory, Morpion (à 2 joueurs), Motus, Osu, Puissance 4 (à 2 joueurs), Snake, TierList**. La Bataille navale se joue **contre l'ordinateur** ; les jeux à 2 joueurs (Go, Inposteur, Morpion, Puissance 4) se jouent **sur le même téléphone**. Le **Morpion** utilise la variante « 3 pions max » : chaque joueur ne garde que ses 3 derniers symboles (le plus ancien, grisé, disparaît quand on en pose un 4ᵉ) — **il y a donc toujours un vainqueur, jamais d'égalité**. L'**Osu** propose une manche de **60 s** et **5 niveaux de difficulté** (du Très facile à l'Extrême : plus c'est dur, plus les cercles apparaissent vite, sont petits et disparaissent tôt). Chaque jeu est jouable au clavier **et** au tactile (glissé, pavé directionnel, clavier AZERTY à l'écran pour Motus). Les records personnels sont mémorisés, ainsi que le dernier jeu ouvert. Accessible aussi **dès la page de connexion** (tuile 🎮 Jeux de la grille de modules, sans connexion Spotify).
